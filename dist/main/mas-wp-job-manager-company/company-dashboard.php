...
<div id="job-manager-company-dashboard">
	<div class="job-manager-company-dashboard-inner">
        <?php if( jobhunt_is_woocommerce_activated() ) {
            do_action( 'woocommerce_account_navigation' );
        } ?>
        <div class="job-manager-company-dashboard-content">
			<p><?php echo esc_html( _n( 'Your company can be viewed, edited or removed below.', 'Your companies can be viewed, edited or removed below.', mas_wpjmc_company_manager_count_user_companies(), 'jobhunt' ) ); ?></p>
			<table class="job-manager-companies">
				...
				<tbody>
					<?php if ( ! $companies ) : ?>
						<tr>
							<td colspan="<?php echo sizeof( $company_dashboard_columns ); ?>"><?php _e( 'You do not have any active company listings.', 'jobhunt' ); ?></td>
						</tr>
					<?php else : ?>
														...
														case 'expired' :
															if ( get_option( 'job_manager_manager_submit_company_form_page_id' ) ) {
																$actions['relist'] = array(
																	'label' => esc_html__( 'Relist', 'jobhunt' ),
																	'nonce' => true
																);
															}
														break;
					...
					<tfoot>
						<tr>
							<td colspan="<?php echo sizeof( $company_dashboard_columns ); ?>">
								<a href="<?php echo esc_url( get_permalink( $submit_company_form_page_id ) ); ?>"><?php _e( 'Add Company', 'jobhunt' ); ?></a>
							</td>
						</tr>
					</tfoot>
				<?php endif; ?>
